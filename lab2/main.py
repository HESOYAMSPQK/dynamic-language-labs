...
def start(message):
    ...
    markup.add(item)
    bot.send_message(message.chat.id, 'Добро пожаловать в игру "История России"!\nНажмите кнопку "Начать игру", чтобы начать.', reply_markup=markup)
...
def ask_question(chat_id, question_index, correct_count):
    ...
    if question:
        ...
        bot.send_message(chat_id, f'Вопрос {question["index"]}: {question["question"]}', reply_markup=markup)
        bot.register_next_step_handler_by_chat_id(chat_id, check_answer, question, correct_count)
    else:
        stop_game(chat_id, correct_count)

def check_answer(message, question, correct_count):
    user_answer = message.text
    correct_answer = question['answers'][question['correct_answer']]
    if user_answer == correct_answer:
        bot.send_message(message.chat.id, 'Правильно! Вы ответили верно.К вашему баллансу прибавляется - 200 000 рублей!!')
        next_question_index = question['index'] + 1
        next_correct_count = correct_count + 1
        if next_correct_count == 5:
            stop_game(message.chat.id, next_correct_count)
        else:
            ask_question(message.chat.id, next_question_index, next_correct_count)
    else:
        bot.send_message(message.chat.id, f'К сожалению, вы ответили неправильно. Правильный ответ: {correct_answer}')
        stop_game(message.chat.id, correct_count)

def stop_game(chat_id, correct_count):
    if correct_count == 5:
        bot.send_message(chat_id, 'Поздравляю! Вы ответили на все вопросы. Вы стали миллионером, введите все данные своей карточки(с обратной стороны тоже) и мы отправим вам ваш выйгрыш!!!!')
    else:
